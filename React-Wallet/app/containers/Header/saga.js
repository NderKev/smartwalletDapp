...
import {odinCoin} from 'utils/contracts/odinCoin'
import {odinWallet} from 'utils/contracts/odinWallet'
import { BeepToken } from 'utils/contracts/BeepToken';
...
//import { BeepToken } from '../../utils/contracts/BeepToken';
//import  {odinCoin} from
const web3 = new Web3(); // eslint-disable-line
const erc20Contract = web3.eth.contract(erc20Abi);
const odinCoinContract = web3.eth.contract(odinCoin);
const BeepTokenContract = web3.eth.contract(BeepToken);
const odinCoinWallet = web3.eth.contract(odinWallet);
//console.log(web3);
//console.log(odinCoinContract);
...
*/
//var tokenContractAddress = "0x3155c32e789fd5a82657bb859f95611ca7fab725";
  ...
  try {
    ...
    if (keystore) {
      ...
      if (action.networkName === 'Ropsten Testnet' && !usedFaucet) {
        yield put(checkFaucet());makeSelectKeystore
      }
      else if (action.networkName === 'Local RPC' && !usedFaucet){
    ...
    if (tokenToSend === 'eth') {
      ...
      function sendTokenPromise(tokenContractAddress, sendToAddress, sendAmount, params) { // eslint-disable-line no-inner-declarations
        return new Promise((resolve, reject) => {
          ...
          const tokenContract = odinCoinContract.at(tokenContractAddress)
